...
export default function EditQuestModal() {
  ...
  const { editQuest, deleteQuest } = useQuestStore();

  // Receive quest data via search params
  ...
  const [category, setCategory]   = useState(params.category ?? 'Health');
  ...
  const [reminderTime, setReminderTime] = useState(params.reminder_time ?? '');
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    const success = await editQuest(params.id, {
      name: name.trim(),
      category,
      difficulty,
      frequency,
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            const success = await deleteQuest(params.id);
            if (success) {
              router.back();
            } else {
  ...
  return (
    <SafeAreaView style={s.root} edges={['top', 'bottom']}>
      ...
      <View style={s.titleBar}>
        <View>
          <Text style={s.titleText}>Edit Quest</Text>
          <Text style={s.titleSub}>Update your challenge</Text>
        </View>
      ...
      >
        ...
        <TouchableOpacity style={s.deleteBtn} onPress={handleDelete} activeOpacity={0.85}>
          <Text style={s.deleteBtnText}>🗑️  DELETE QUEST</Text>
        </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  deleteBtnText: {
    ...
    color: Colors.error, letterSpacing: 0.5,
  },
